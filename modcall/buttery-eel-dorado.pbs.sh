...
/usr/bin/time -v  buttery-eel -i ${MERGED_SLOW5} -o ${BASECALL_OUT}/reads.sam -g ${ONT_DORADO_PATH} --port ${PORT} --use_tcp --model ${MODEL} --modbase_models ${MOD_MODEL} -x cuda:all --slow5_threads 10 --procs 20 --call_mods  || die "basecalling failed"
